...
const InterestLoanPendingViewPage = ({ params: paramsPromise }) => {
  ...
  const handleUpdateStatus = async () => {
    try {
      setUpdating(true);
      await interestLoanService.updateLoan(loan.loanId, {
      ...
      const redirectPath =
        fromPage === "followup"
          ? "/admin/interest-loan/followups"
          : "/admin/interest-loan/pending";
  ...
  const handleSaveEMI = async (e) => {
    ...
    try {
      ...
      await interestLoanService.payInterestEMI(selectedEmi._id, payload);
      showToast("Interest payment updated successfully", "success");
      setShowModal(false);
      const redirectPath =
        fromPage === "followup"
          ? "/admin/interest-loan/followups"
      ...
      <div className={`min-h-screen bg-[#F8FAFC] flex transition-colors duration-300 ${isDarkMode ? "interest-pending-view-dark-mode" : ""}`}>
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <main className="py-8 px-4 sm:px-8">
                ...
                <div className="lg:col-span-2 space-y-8">
                  ...
                  <div className="bg-white rounded-3xl border border-slate-200 p-8 shadow-sm">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      ...
                      {(loan.guarantorName || loan.guarantorMobileNumbers?.length > 0) && (
                        <div>
                          <span className="text-[9px] font-black text-primary uppercase tracking-widest block mb-1">
                            Guarantor
                          </span>
                          ...
                          <div className="space-y-2 mt-1">
                            {(Array.isArray(loan.guarantorMobileNumbers) ? loan.guarantorMobileNumbers : []).filter(Boolean).map((num, idx) => (
                              <button
                                key={idx}
                                onClick={(e) => {
                                  const rect = e.currentTarget.getBoundingClientRect();
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                      <div className="space-y-2">
                        ...
                        <textarea
                          ...
                          className="w-full bg-slate-800 border border-slate-700 rounded-2xl p-4 text-white text-sm font-medium focus:outline-none focus:border-primary transition-all min-h-[100px] placeholder:text-slate-600 resize-none uppercase"
                        />
                      </div>
                      <div className="space-y-2">
                          ...
                          <>
                            <div className="flex justify-between items-center mb-1">
                              ...
                              <span className="text-xs font-black text-slate-600 font-mono">
                                {pendingEmis.length}{" "}
                                {pendingEmis.length === 1 ? "Month" : "Months"}
                              </span>
                            </div>
                        ...
                        {pendingEmis.map((emi) => (
                          ...
                          >
                            <div className="flex justify-between items-center mb-2">
                              <span className={`text-[8px] font-black uppercase tracking-widest px-2 py-0.5 rounded-full ${emi.status === "Partially Paid" ? "bg-orange-50 text-orange-600" : "bg-red-50 text-red-600"}`}>
                                {emi.status}
                              </span>
                  ...
                  <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                    <div>
                      <h3 className="text-xl font-black text-slate-900 uppercase tracking-tight">
                        UPDATE INTEREST #{selectedEmi?.emiNumber}
                      </h3>
                  ...
                  >
                    ...
                    <div className="p-8 border-t border-slate-100 bg-slate-50/50 flex gap-4">
                      <button
                        type="button"
                        onClick={() => setShowModal(false)}
                        className="flex-1 px-6 py-4 bg-white border border-slate-200 text-slate-400 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-slate-50 transition-all"
                      >
                        Cancel
                      </button>
                      <button
                        type="submit"
                        disabled={updating}
                        className="flex-[2] px-6 py-4 bg-primary text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl shadow-blue-500/20 hover:bg-blue-700 transition-all disabled:opacity-50"
                      >
            ...
            {/* Contact Action Menu */}
            <ContactActionMenu
              isOpen={!!activeContactMenu}
              anchorEl={activeContactMenu}
...
export default InterestLoanPendingViewPage;
