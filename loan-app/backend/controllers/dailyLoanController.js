...
exports.createDailyLoan = asyncHandler(async (req, res, next) => {
  ...
  // No processing fee for daily loans as per business rules
...
exports.getDailyLoanROI = asyncHandler(async (req, res, next) => {
  const loan = await DailyLoan.findById(req.params.id).lean();
  if (!loan) {
    return next(new ErrorHandler("Daily loan not found", 404));
  }
  const emis = await EMI.find({ loanId: loan._id, loanModel: "DailyLoan" }).lean();
  ...
  sendResponse(res, 200, "success", "Daily loan ROI calculated", null, roi);
});
...
exports.updateDailyLoan = asyncHandler(async (req, res, next) => {
  ...
  if (!isSuperAdmin) {
    ...
    await notifyAdmins({
      ...
      message: `Employee ${req.user.name} requested changes to loan ${dailyLoan.loanNumber} (${dailyLoan.customerName || "—"}). ${changes.length} field(s) changed.`,
      data: { loanNumber: dailyLoan.loanNumber, changes },
    });

    return sendResponse(res, 200, "success", "Changes submitted for approval", null, dailyLoan);
  }
